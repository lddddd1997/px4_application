#ifndef PX4_APPLICATION_FUNC_UTILS_H_
#define PX4_APPLICATION_FUNC_UTILS_H_
...
public:
    ...
    static double GetDeltaTimeMs(const ros::Time& last_time);
private:
};
...
double FunctionUtils::GetDeltaTimeMs(const ros::Time& last_time)
{
    ros::Time current_time = ros::Time::now();
    double current_time_sec = current_time.sec - last_time.sec;
    double current_time_msec = (current_time.nsec - last_time.nsec) / 1e6;
    return (current_time_sec * 1000.0 + current_time_msec);
}
