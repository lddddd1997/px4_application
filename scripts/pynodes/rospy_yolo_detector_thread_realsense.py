...
def Detect(imgae_input, depth_frame_input):
    # for path, img, im0s, vid_cap, s in dataset:

    dataset = LoadImagesInfo(imgae_input)
    path = dataset[0]
    img = dataset[1]
    im0s = dataset[2]
    vid_cap = dataset[3]
    ...
    for i, det in enumerate(pred):  # per image
        ...
        if len(det):
            ...
            if SOCKET_ON:
                ...
                for target_num, target_info in enumerate(target_det): # 处理检测信息
                    ...
                    # realsense 3d检测
                    [pixel_x, pixel_y] = [int((target_info[0] + target_info[2]) / 2), int((target_info[1] + target_info[3]) / 2)] # 可能中心点未落在目标上，待改进
                    ...
                    for delta_x in range(int(-bbox_w / 4), int(bbox_w / 4)):
                        for delta_y in range(int(-bbox_h / 4), int(bbox_h / 4)): # 扫描中心点周围半个边框大的区域
                            scan_dist = depth_frame_input.get_distance(pixel_x + delta_x, pixel_y + delta_y)
                            if scan_dist != 0:
                    ...
                    cv2.circle(im0, (real_pixel_x, real_pixel_y), 3, (255, 0, 0), 5) # 画出目标中心点
        ...
        #     print("no target detected...")
            if SOCKET_ON:
                ...
                cv2.putText(im0, "fps: " + str(round(detect_fps, 3)), (5, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
...
if __name__ == "__main__":
    ...
    while not rospy.is_shutdown():
        ...
        # cv2.waitKey(1)
        Detect(color_image_raw, depth_frame)
