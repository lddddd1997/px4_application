...
# 采集local_position、uav_command、flat_setpoint
# 开启保存检测视频
...
--tab -e 'bash -c "sleep 10; roslaunch px4_application base_form_publisher.launch uav_id:=$uav_id; exec bash"' \
--tab -e 'bash -c "sleep 10; roslaunch px4_application det_tensorrt_yolo_realsense.launch uav_id:=$uav_id; exec bash"' \
