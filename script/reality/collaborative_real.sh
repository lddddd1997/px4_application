...
--tab -e 'bash -c "sleep 4; roslaunch px4_application base_uav_control.launch uav_id:=$uav_id; exec bash"' \
--tab -e 'bash -c "sleep 4; roslaunch px4_application app_uav_collaboration.launch uav_id:=$uav_id; exec bash"' \
--tab -e 'bash -c "sleep 5; roslaunch MonoCamera findObject.launch uav_id:=$uav_id; exec bash"' \
