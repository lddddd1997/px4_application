                     ...
                     States** _State)
{
    ...
    if(_current_info.uav_status.extended_state.landed_state == mavros_msgs::ExtendedState::LANDED_STATE_ON_GROUND)
    {
        if(this->takeoff_id)
        {
            ...
            // ros::NodeHandle nh;
            // if(nh.getNamespace() == "/uav2")
            // {
            //     this->takeoff_position.x = _current_info.uav_status.local_position.x + 2.0;
            //     this->takeoff_position.y = _current_info.uav_status.local_position.y - 2.0;
            // }
            // else if(nh.getNamespace() == "/uav3")
            // {
            //     this->takeoff_position.x = _current_info.uav_status.local_position.x;
            //     this->takeoff_position.y = _current_info.uav_status.local_position.y - 1.0;
            // }
            // else if(nh.getNamespace() == "/uav4")
            // {
            //     this->takeoff_position.x = _current_info.uav_status.local_position.x - 1.0;
            //     this->takeoff_position.y = _current_info.uav_status.local_position.y - 1.0;
            // }
...
TakeOff::TakeOff()// : acc_visualization(nh_private, "acc_visualization")
{
    ros::NodeHandle nh("~");
    ...
    switch(this->own_id)
    {
        ...
        case OtherSubscriber::uav_1 + 1:
        {
            ...
            for(int i = 15, j = 1; i <= 360; i += 15, j++)
                this->route.col(j) = Eigen::Vector3d(5.0 + -5.0 * cos(i / 57.3), -5.0 * sin(i / 57.3), _current_info.uav_status.local_position.z);
        ...
        case OtherSubscriber::uav_2 + 1:
        {
            ...
            // 通信编队，保持在2m后
            // this->leader_status.position.x = total_info.uav_status[OtherSubscriber::uav_1].local_position.x + 2.0 - 3.0 * cos_yaw; // +3.0为1号机相对于2号机的初始摆放偏差
            // this->leader_status.position.y = total_info.uav_status[OtherSubscriber::uav_1].local_position.y - 3.0 * sin_yaw;
            // this->leader_status.position.z = total_info.uav_status[OtherSubscriber::uav_1].local_position.z + 2.0;
            ...
            {
                this->leader_status.position.x = _current_info.tf_drone_status.pose.position.x - 2.0 * cos_yaw;
                this->leader_status.position.y = _current_info.tf_drone_status.pose.position.y - 2.0 * sin_yaw;
            ...
            double ex = this->leader_status.position.x - _current_info.uav_status.local_position.x;
            double ey = this->leader_status.position.y - _current_info.uav_status.local_position.y;
            // std::cout << sqrt(ex * ex + ey * ey) << std::endl;
            // geometry_msgs::Vector3 acc;
            // acc.x = _current_info.tf_drone_status.pose.position.x;
            // acc.y = _current_info.tf_drone_status.pose.position.y;
            // acc.z = sqrt(ex * ex + ey * ey);
            // acc_visualization.PublishMessage(acc);
            /*任务指令*/
            _command_deliver->header.stamp = ros::Time::now();
        ...
        //         float circle_radius = 8.0;
        //         float omega = fabs(linear_vel / circle_radius);

        //         const float angle = ttt * omega;
        //         const float cos_angle = cos(angle);
        //         const float sin_angle = sin(angle);

        //         _command_deliver->x = circle_radius * sin_angle + 0;
        //         _command_deliver->y = circle_radius * -cos_angle + circle_radius;
        //         _command_deliver->yaw = angle;
        //         // _command_deliver->x = 0;
        //         // _command_deliver->y = ttt * 1.0;
        ...
        //     //
        //     _command_deliver->header.stamp = ros::Time::now();
        //     _command_deliver->command_type = px4_application::UavCommand::NORMAL; // 轨迹跟踪模式，控制指令由traj_publisher节点发布给uav_control
        //     _command_deliver->period = 0.05;
        //     _command_deliver->update = _current_info.door_status.update;
        //     _command_deliver->xyz_id = px4_application::UavCommand::PX_PY_PZ;
        //     _command_deliver->yaw_id = px4_application::UavCommand::YAW;
        ...
        //     // 通信编队，保持在2m后
        //     this->leader_status.position.x = total_info.uav_status[OtherSubscriber::uav_1].local_position.x + 2.0 - 2.0 * cos_yaw; // +3.0为1号机相对于2号机的初始摆放偏差
        //     this->leader_status.position.y = total_info.uav_status[OtherSubscriber::uav_1].local_position.y - 2.0 * sin_yaw;
        ...
        //     acc.z = sqrt(ex * ex + ey * ey);
        //     acc_visualization.PublishMessage(acc);
        //     Eigen::Vector3d out = FormationOutput(total_info, OtherSubscriber::uav_2);
        //     /*任务指令*/
        //     _command_deliver->header.stamp = ros::Time::now();
        //     _command_deliver->command_type = px4_application::UavCommand::NORMAL; // 轨迹跟踪模式，控制指令由form_publisher节点发布给uav_control
        //     _command_deliver->period = 0.05;
        ...
        //     _command_deliver->yaw_id = px4_application::UavCommand::YAW;
        //     _command_deliver->frame_id = px4_application::UavCommand::LOCAL;
        //     _command_deliver->x = out[0];
        //     _command_deliver->y = out[1];
        //     _command_deliver->z = leader_status.position.z;
        //     _command_deliver->yaw = leader_status.yaw;
        //     _command_deliver->task_name = "2_Mission";
        //     _uav_command_pub.publish(*_command_deliver);
        ...
        //     // 通信编队，保持在2m后
        //     this->leader_status.position.x = total_info.uav_status[OtherSubscriber::uav_1].local_position.x + 4.0 - 2.0 * cos_yaw - 2.0 * sin_yaw; // +3.0为1号机相对于2号机的初始摆放偏差
        //     this->leader_status.position.y = total_info.uav_status[OtherSubscriber::uav_1].local_position.y - 2.0 * sin_yaw + 2.0 * cos_yaw;
        //     this->leader_status.position.z = total_info.uav_status[OtherSubscriber::uav_1].local_position.z;
        //     this->leader_status.velocity.x = total_info.uav_status[OtherSubscriber::uav_1].local_velocity.x;
        //     this->leader_status.velocity.y = total_info.uav_status[OtherSubscriber::uav_1].local_velocity.y;
        //     this->leader_status.velocity.z = total_info.uav_status[OtherSubscriber::uav_1].local_velocity.z;
        ...
        //     acc.z = sqrt(ex * ex + ey * ey);
        //     acc_visualization.PublishMessage(acc);
        //     Eigen::Vector3d out = FormationOutput(total_info, OtherSubscriber::uav_3);
        //     /*任务指令*/
        //     _command_deliver->header.stamp = ros::Time::now();
        //     _command_deliver->command_type = px4_application::UavCommand::NORMAL; // 轨迹跟踪模式，控制指令由form_publisher节点发布给uav_control
        //     _command_deliver->period = 0.05;
        ...
        //     // 通信编队，保持在2m后
        //     this->leader_status.position.x = total_info.uav_status[OtherSubscriber::uav_1].local_position.x + 6.0 - 2.0 * cos_yaw + 2.0 * sin_yaw; // +3.0为1号机相对于2号机的初始摆放偏差
        //     this->leader_status.position.y = total_info.uav_status[OtherSubscriber::uav_1].local_position.y - 2.0 * sin_yaw - 2.0 * cos_yaw;
        //     this->leader_status.position.z = total_info.uav_status[OtherSubscriber::uav_1].local_position.z;
        //     this->leader_status.velocity.x = total_info.uav_status[OtherSubscriber::uav_1].local_velocity.x;
        //     this->leader_status.velocity.y = total_info.uav_status[OtherSubscriber::uav_1].local_velocity.y;
        ...
        //     acc.z = sqrt(ex * ex + ey * ey);
        //     acc_visualization.PublishMessage(acc);
        //     Eigen::Vector3d out = FormationOutput(total_info, OtherSubscriber::uav_4);
        //     /*任务指令*/
        //     _command_deliver->header.stamp = ros::Time::now();
        //     _command_deliver->command_type = px4_application::UavCommand::NORMAL; // 轨迹跟踪模式，控制指令由form_publisher节点发布给uav_control
        //     _command_deliver->period = 0.05;
