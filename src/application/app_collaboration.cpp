...
#include "application/app_collaboration.h"
...
UavCollaboration::UavCollaboration(const ros::NodeHandle& _nh, double _period) : RosBase(_nh, _period), current_info("mission")
{
    Initialize();
}
    ...
    {   
        switch(this->own_id)
        {
            ...
            case OtherSubscriber::uav_1 + 1:
            {
                ...
                // _command_deliver->yaw_id = px4_application::UavCommand::YAW;
                // _command_deliver->frame_id = px4_application::UavCommand::BODY;
                // _command_deliver->x = -this->TrackingX.ControlOutput(this->tracking_position.x, _current_info.target_status.pcl_position.x);
                // _command_deliver->y = -this->TrackingY.ControlOutput(this->tracking_position.y, _current_info.target_status.pcl_position.y);
                // _command_deliver->z = -this->TrackingZ.ControlOutput(this->tracking_position.z, _current_info.target_status.pcl_position.z);
                // // _command_deliver->yaw = lock_yaw_;
                ...
                //     FunctionUtils::DataFileWrite(this->tracking_position, _current_info.target_status.pcl_position, this->saved_file_path + "camera_tracking.txt");
...
{
    ros::init(argc, argv, "app_collaboration");
    ros::NodeHandle nh;
